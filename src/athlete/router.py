...
async def get_latest_athlete(
    ...
    position_names = (
        ", ".join([p.name for p in latest_athlete.positions])
        if latest_athlete.positions
        else "N/A"
    )
...
async def list_all_athletes_for_selection(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_async_session),
):
...
async def get_athlete_skills_data(
    ...
    return await get_athlete_skill_progression(
        user_id=current_user.id, athlete_uuid=athlete_uuid, db=db
    )


@router.post(
    "/{athlete_uuid}/recalculate-skills", response_model=AthleteSkillProgression
)
...
async def delete_athlete_profile(
    athlete_uuid: UUID,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_async_session),
):
...
async def upload_athlete_profile_image(
    athlete_uuid: UUID,
    ...
    return {
        "message": "Athlete profile image uploaded successfully",
        "image_url": image_url,
    }


@router.delete("/{athlete_uuid}/image")
async def delete_athlete_profile_image(
    athlete_uuid: UUID,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_async_session),
):
