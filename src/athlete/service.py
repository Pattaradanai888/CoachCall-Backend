...
from .models import Athlete, AthleteSkill, ExperienceLevel, Group, Position
...
async def create_athlete(user_id: int, athlete: AthleteCreate, db: AsyncSession):
    athlete_data = athlete.model_dump(
        exclude={"group_ids", "position_ids", "experience_level_id"}
    )
    ...
    await db.refresh(
        db_athlete,
        attribute_names=["groups", "positions", "experience_level", "skill_levels"],
    )
    result = await db.execute(
        select(Athlete)
        .where(Athlete.id == db_athlete.id)
        .options(
...
async def get_coach_athletes(
    user_id: int, db: AsyncSession, skip: int = 0, limit: int = 5
) -> Sequence[Athlete]:
    result = await db.execute(
        select(Athlete)
...
async def get_all_coach_athletes_for_selection(
    ...
    result = await db.execute(
        ...
        .options(selectinload(Athlete.positions), selectinload(Athlete.groups))
    )
    return result.scalars().all()


async def get_coach_athlete_by_uuid(
    user_id: int, athlete_uuid: _uuid.UUID, db: AsyncSession
):
    query = (
        select(Athlete)
...
async def update_athlete(
    user_id: int, athlete_uuid: _uuid.UUID, athlete: AthleteUpdate, db: AsyncSession
):
...
async def upload_athlete_image(
    user_id: int, athlete_uuid: _uuid.UUID, file: UploadFile, db: AsyncSession
) -> str:
...
async def delete_athlete_image(
    user_id: int, athlete_uuid: _uuid.UUID, db: AsyncSession
) -> None:
...
async def get_latest_athlete_for_coach(
    user_id: int, db: AsyncSession
) -> Athlete | None:
    query = (
        select(Athlete)
