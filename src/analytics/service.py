...
from sqlalchemy import func, select, desc, Date
from sqlalchemy.dialects.postgresql import insert as pg_insert
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy.orm import selectinload

from src.athlete.models import Athlete, AthleteSkill
from src.course.models import TaskCompletion, Task, TaskSkillWeight, Skill, Session, Course, SessionTask
from .schemas import AthleteSkillProgression, SkillScore
...
async def get_athlete_stats(user_id: int, db: AsyncSession):
    ...
    total_athletes = await db.scalar(select(func.count(Athlete.id)).where(Athlete.user_id == user_id))
    ...
    for date in dates:
        ...
        trend_data.append({
            "date": date.isoformat(), "day_name": date.strftime("%a"),
            "formatted_date": date.strftime("%m/%d"), "count": count
        })
    ...
    prev_week_count = await db.scalar(
        select(func.count(Athlete.id)).where(
            Athlete.user_id == user_id, Athlete.created_at >= prev_week_start_dt,
            Athlete.created_at < seven_days_ago_start
        )
    )
    week_change = None
    ...
    return {
        "today": today_count or 0, "week": week_count or 0, "month": month_count or 0,
        "total": total_athletes or 0, "trend": [item["count"] for item in trend_data],
        "trend_detailed": trend_data,
        "insights": {"week_change_percent": week_change, "peak_day": peak_day, "avg_daily": avg_daily, "is_growing": is_growing}
    }


async def get_athlete_skill_progression(user_id: int, athlete_uuid: PyUUID, db: AsyncSession) -> AthleteSkillProgression:
    athlete_q = await db.execute(
        select(Athlete).where(Athlete.uuid == athlete_uuid, Athlete.user_id == user_id)
        .options(selectinload(Athlete.skill_levels).selectinload(AthleteSkill.skill))
    )
    athlete = athlete_q.scalar_one_or_none()
    if not athlete:
        raise HTTPException(status_code=404, detail="Athlete not found or you do not have permission.")

    all_user_skills_q = await db.execute(select(Skill).where(Skill.user_id == user_id).order_by(Skill.id))
    ...
    if completion_dates:
        ...
        day_one_completions_q = await db.execute(
            ...
            .where(TaskCompletion.athlete_id == athlete.id, func.cast(TaskCompletion.completed_at, Date) == first_date)
            .options(selectinload(TaskCompletion.task).selectinload(Task.skill_weights).selectinload(TaskSkillWeight.skill))
        )
        day_one_completions = day_one_completions_q.scalars().unique().all()
        skill_totals = defaultdict(lambda: {'total_weighted_score': 0.0, 'total_weight': 0.0})
        for comp in day_one_completions:
            if not comp.task or not comp.scores_breakdown: continue
            for sw in comp.task.skill_weights:
                if sw.skill_id in all_user_skills and str(sw.skill_id) in comp.scores_breakdown:
    ...
    day_one_scores = [
        SkillScore(skill_id=skill_id, skill_name=all_user_skills[skill_id], average_score=score if score is not None else 0.0)
        for skill_id, score in sorted(day_one_scores_dict.items())
    ]

    athlete_current_scores_map = {ath_skill.skill_id: float(ath_skill.current_score) for ath_skill in athlete.skill_levels}
    current_scores = [
        SkillScore(skill_id=skill_id, skill_name=skill_name, average_score=athlete_current_scores_map.get(skill_id, 0.0))
...
async def calculate_ema_skill_scores(
    ...
    for session_id, session_data in sorted_sessions:
        ...
        session_skill_avg_data = defaultdict(lambda: {'total_weighted_score': 0.0, 'total_weight': 0.0})
        ...
        for comp in session_data['completions']:
            ...
            for sw in comp.task.skill_weights:
                if str(sw.skill_id) in comp.scores_breakdown:
                    score = comp.scores_breakdown[str(sw.skill_id)]
                    weight = float(sw.weight)
                    session_skill_avg_data[sw.skill_id]['total_weighted_score'] += score * weight
        ...
        for skill_id, data in session_skill_avg_data.items():
            if data['total_weight'] > 0:
                ...
                if skill_id not in current_ema_scores:
                    ...
                    current_ema_scores[skill_id] = (session_avg * alpha) + (current_ema_scores[skill_id] * (1 - alpha))
...
async def update_athlete_skill_scores(athlete_id: int, db: AsyncSession):
    ...
    upsert_values = [
        {"athlete_id": athlete_id, "skill_id": skill_id, "current_score": round(score, 2)}
