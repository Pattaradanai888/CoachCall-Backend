...
from src.analytics.service import update_athlete_skill_scores, calculate_ema_skill_scores
# --- Corrected and Verified Imports ---
from src.athlete.models import Athlete, AthleteSkill
from src.course.models import Course, Session, Task, Skill, SessionTask, TaskSkillWeight, TaskCompletion
from src.course.schemas import (
    CourseCreate, SkillCreate, SessionCreate, SessionCompletionPayload, EventItem,
    CourseArchiveStatusUpdate, SessionSkillComparison
)
...
async def create_course(user_id: int, course_data: CourseCreate, db: AsyncSession) -> Course:
    ...
    for session_data in session_data_list:
        task_data_list = session_data.tasks
        session_dict = session_data.model_dump(exclude={'tasks'})
        db_session = Session(**session_dict, user_id=user_id, course=db_course, status="To Do")
        for i, task_data in enumerate(task_data_list):
            skill_weights_data = task_data.skill_weights
            db_task = Task(**task_data.model_dump(exclude={'skill_weights'}), user_id=user_id)
            db_session.tasks.append(SessionTask(task=db_task, sequence=i + 1))
            for sw_data in skill_weights_data:
                db_task.skill_weights.append(TaskSkillWeight(**sw_data.model_dump()))
...
async def confirm_session(user_id: int, session_id: int, db: AsyncSession) -> Session:
    ...
    if not updated_id:
        raise HTTPException(status_code=404, detail="Pending session not found or you do not have permission.")
    await db.commit()
...
async def cleanup_pending_sessions(db: AsyncSession):
    ...
    await db.execute(
        delete(TaskCompletion).where(TaskCompletion.session_id.in_(session_ids_to_delete))
    )

    await db.execute(
        delete(Session).where(Session.id.in_(session_ids_to_delete))
    )
...
async def delete_course(user_id: int, course_id: int, db: AsyncSession) -> dict:
    ...
    if not db_course:
        raise HTTPException(status_code=404, detail="Course not found or you do not have permission.")
...
async def update_course_archive_status(user_id: int, course_id: int, status_update: CourseArchiveStatusUpdate,
    ...
    stmt = update(Course).where(Course.id == course_id, Course.user_id == user_id).values(
        is_archived=status_update.is_archived)
    result = await db.execute(stmt)
...
async def update_session_status(user_id: int, session_id: int, new_status: str, db: AsyncSession) -> Session | None:
    ...
    await db.execute(update(Session).where(Session.id == subquery).values(**values_to_update))
    await db.commit()
...
async def save_task_completions(user_id: int, session_id: int, payload: SessionCompletionPayload, db: AsyncSession):
    ...
    for completion_data in payload.completions:
        ...
        if athlete_id:
            db_completions.append(TaskCompletion(
    ...
    athlete_int_ids = {athletes_map[str(uuid)] for uuid in athlete_uuids if str(uuid) in athletes_map}
...
async def get_session_report_data(user_id: int, session_id: int, db: AsyncSession) -> Optional[dict]:
    ...
    all_user_skills_q = await db.execute(select(Skill).where(Skill.user_id == user_id).order_by(Skill.name))
    all_user_skills = all_user_skills_q.scalars().all()

    evaluations = {}
    participating_athlete_objects = {}
    for completion in session.completions:
        if not completion.athlete: continue
    ...
    for athlete_uuid, athlete in participating_athlete_objects.items():
        ...
        before_scores_dict = await calculate_ema_skill_scores(db, athlete_internal_id, exclude_session_id=session_id)

        after_scores_dict = await calculate_ema_skill_scores(db, athlete_internal_id, exclude_session_id=None)

        before_scores_list = [
            SkillScore(skill_id=skill.id, skill_name=skill.name, average_score=before_scores_dict.get(skill.id, 0.0))
            for skill in all_user_skills
        ]
        after_scores_list = [
            SkillScore(skill_id=skill.id, skill_name=skill.name, average_score=after_scores_dict.get(skill.id, 0.0))
            for skill in all_user_skills
        ]

        skill_comparison_data[str(athlete_uuid)] = SessionSkillComparison(
            before=before_scores_list,
            after=after_scores_list
        )
