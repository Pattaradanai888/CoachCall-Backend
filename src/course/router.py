...
from .service import (
    ...
    update_course,
    update_course_archive_status,
    update_course_attendees,
    update_session,
...
async def list_sessions(
    ...
    sessions = await get_sessions(
        user_id=current_user.id, is_template=is_template, db=db
    )
    return sessions
...
async def create_new_session(
    ...
    return await create_session(
        user_id=current_user.id, session_data=session_data, db=db
    )
...
async def get_single_session(
    ...
    session = await get_session_by_id(
        user_id=current_user.id, session_id=session_id, db=db
    )
    if not session:
        raise HTTPException(
...
async def update_existing_session(
    ...
    return await update_session(
        user_id=current_user.id, session_id=session_id, session_data=session_data, db=db
    )
...
async def update_existing_course(
    ...
    return await update_course(
        user_id=current_user.id, course_id=course_id, course_data=course_data, db=db
    )
...
async def update_course_status(
    ...
    return await update_course_archive_status(
        user_id=current_user.id, course_id=course_id, status_update=status_update, db=db
    )
...
async def upload_a_course_cover_image(
    ...
    image_url = await upload_course_image(
        user_id=current_user.id, course_id=course_id, file=file, db=db
    )
    return {"url": image_url}


@router.put("/{course_id}/athletes", response_model=CourseRead)
async def update_course_athletes(
    course_id: int,
    athlete_uuids: list[UUID],
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_async_session),
):
...
async def get_session_report(
    ...
    report_data = await get_session_report_data(
        user_id=current_user.id, session_id=session_id, db=db
    )
    if not report_data:
        raise HTTPException(
            status_code=404,
            detail="Session report not found or session is not complete.",
        )
    return report_data
...
async def complete_session_and_save_scores(
    ...
    await save_task_completions(
        user_id=current_user.id, session_id=session_id, payload=payload, db=db
    )
